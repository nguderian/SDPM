...
class teamInfo {
    ...
    static async createTeam(req,res,next){
        ...
        const { user_id, project_name, description, class_id, sponsor_name, sponsor_company, sponsor_email } = req.body;
        let user_type;
        ...
        try{
            ...
            { replacements : [description, project_name, user_id, sponsor_name, sponsor_company, sponsor_email, class_id],
            type : sequelize.QueryTypes.CALL});
    ...
    static async getTeamID(req, res, next){
        ...
        let team_id = teamid[0].team_id;
        let resultTeam;
        try{
            resultTeam = await sequelize.query('CALL get_team(?)',
            { replacements : [team_id], type: sequelize.QueryTypes.CALL });
        } catch(error)
        {
            console.log("get team failed");
            res.send({ status : "get team failed"});
        }
